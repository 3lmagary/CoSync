...
if (glo[importIdentifier] === true) {
  ...
  console.trace("DUPLICATE IMPORT TRACE IN ESM");
}
...
var CoSyncPlugin = class extends import_obsidian.Plugin {
  ...
  updateStatusBar(status, customText) {
    ...
    container.title = "CoSync Collaboration Status";
  ...
  async resolveDocumentId(file) {
    try {
      ...
      if (matchByTitle) {
        ...
        const contentWithId2 = injectCosyncId(fileContent2, docId);
        this.isApplyingRemoteUpdate = true;
        this.programmedModifications.add(file.path);
        try {
          await this.app.vault.modify(file, contentWithId2);
        } catch (err) {
    ...
    try {
      ...
      for (const doc2 of serverDocs) {
        ...
        try {
          ...
          new import_obsidian.Notice(`CoSync: Created new local note: "${docTitle}"`);
        } catch (err) {
...
var CoSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Obsidian Connection Code").setDesc("Paste the Connection Code from the CoSync web app here to configure the server, token, and workspace in one click.").addTextArea((text2) => text2.setPlaceholder("Paste your connection code here...").setValue(this.plugin.settings.connectionCode).onChange(async (value) => {
